...
import 'package:sellerkit/Services/Leadchecklistupdate/checklistupdateApi.dart';
import 'package:sellerkit/Services/OrdergiftApi/orderpricecheckApi.dart';
import 'package:sellerkit/Services/PostQueryApi/LeadsApi/LeadCheckListApi.dart';
...
import '../EnquiryController/newenq_controller.dart';
...
class LeadTabController extends ChangeNotifier {
  ...
  checklistverify() {
    for (int i = 0; i < leadChecklistData.length; i++) {
      for (int ij = 0; ij < getleadcheckdatas.length; ij++) {
        if (getleadcheckdatas[ij].Code == leadChecklistData[i].CheckListCode) {
          if (getleadcheckdatas[ij].descriptionTypes == '@') {
            // getleadcheckdatas[ij].textdata =
            //     leadChecklistData[i].CheckListValue;
                mytextcontroller2[ij].text =leadChecklistData[i].CheckListValue.toString();
          } else if (getleadcheckdatas[ij].descriptionTypes != null &&
              getleadcheckdatas[ij].descriptionTypes != "@") {
            getleadcheckdatas[ij].valuechoosen =
                leadChecklistData[i].CheckListValue;
          } else {
            getleadcheckdatas[ij].ischecked = true;
          }
        }
      }
    }
  ...
  checkpopup(BuildContext context, ThemeData theme, int? leadid) {
    showDialog(
        context: context,
        builder: (_) {
          return AlertDialog(
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: ListView.builder(
                          ...
                          itemCount: getleadcheckdatas.length,
                                          ...
                                          getleadcheckdatas[index]
                                              ...
                                              : getleadcheckdatas[index]
                                                  ...
                                                  ? Container(
                                                      ...
                                                      child: TextFormField(
                                                          controller:mytextcontroller2[index],
                                                        minLines: 1,
                                                        maxLines: 5,
                                                        onChanged: (val) {
                                                          st(() {
...
List<TextEditingController> mytextcontroller2 =
      List.generate(60, (i) => TextEditingController());
  List<tabchecklistdata> leadcheccklistfinal = [];
  String? forwardSuccessMsg2 = '';
  int ik=0;
  savechecklist(int? leadid, BuildContext context) async {
    Config config = Config();
    leadcheccklistfinal.clear();
    forwardSuccessMsg2 = '';
     ik = 0;
    for (int i = 0; i < leadcheckdatas.length; i++) {
      if (leadcheckdatas[i].ischecked == true ||
          leadcheckdatas[i].valuechoosen != null ||
          leadcheckdatas[i].textdata != null) {
        var chosenValue = leadcheckdatas[i].valuechoosen ??
            (leadcheckdatas[i].textdata ?? null);
        leadcheccklistfinal.add(tabchecklistdata(
            checkListCode: leadcheckdatas[i].Code,
            checkListValue: leadcheckdatas[i].ischecked == true?'Yes':chosenValue,
            createdDateTime: config.currentDate(),
            leadId: leadid!,
            lineNum: ik + 1,

            // updatedBy: ConstantValues.slidevalue,
      ...
      updateFollowUpDialog = false;
      // Navigator.pop(context);
      await ChecklistupdateApi.getData(leadcheccklistfinal).then((value) {
        if (value.stCode >= 200 && value.stCode <= 210) {
          forwardSuccessMsg = "success";
          forwardSuccessMsg2 = "Lead Checklist Updated Successfully";
  ...
  clearAllListData() {
    ...
    for (var controller in mytextcontroller2) {
      controller.clear();
    }
    forwardSuccessMsg = '';
    forwardSuccessMsg2 = '';
...
//  GetAllEnqData.clear();
  ...
  mapValues(List<GetAllLeadData> leadcheckdata) async {
    for (int i = 0; i < leadcheckdata.length; i++) {
      if (leadcheckdata[i].Status == "Open") {
        ...
        leadOpenAllData.sort((a, b) => b.LeadNum!.compareTo(a.LeadNum!));
        ...
        leadClosedAllData.sort((a, b) => b.LeadNum!.compareTo(a.LeadNum!));
        ...
        leadLostAllData.sort((a, b) => b.LeadNum!.compareTo(a.LeadNum!));
  ...
  bool viewchecklistdialog = false;
...
//  }
  ...
  List<GetLeadCheckData> get getleadleadChecklistData => leadChecklistData;
...
// leadcheckdatas[i].listcontroller.add(TextEditingController());
                ...
                leadcheckdatas[i].descitems =
                    leadcheckdatas[i].descriptionTypes!.split(',');
              }
            }
...
// log("leadDeatilsLData::"+leadDeatilsLData[4].Feedback.toString());
          ...
          if (value.leadDeatilheadsData!.leadChecklistData != null) {
            leadChecklistData = value.leadDeatilheadsData!.leadChecklistData!;
            checklistverify();
          }
          log("leadChecklistData:::" + leadChecklistData.length.toString());
          leadLoadingdialog = false;
          leadForwarddialog = false;
          updateFollowUpDialog = false;
          viewDetailsdialog = true;
  ...
  double? unitPrice;
  double? quantity;
  mapvaluestovisitplan(
      GetAllLeadData leadOpenAllData, BuildContext context) async {
    NewVisitplanController.datafromlead.clear();
    NewVisitplanController.datafromlead.add(leadOpenAllData.Mobile!);
    NewVisitplanController.datafromlead.add(leadOpenAllData.CustomerName!);
    NewVisitplanController.datafromlead.add(leadOpenAllData.add1!.toString());
    NewVisitplanController.datafromlead.add(leadOpenAllData.add2!.toString());
    NewVisitplanController.datafromlead
        .add(leadOpenAllData.pincode!.toString());
    NewVisitplanController.datafromlead
        .add(leadOpenAllData.City!.toString()); //isSelectedCsTag
    NewVisitplanController.datafromlead.add(leadOpenAllData.LeadNum.toString());
    NewVisitplanController.datafromlead
        .add(leadOpenAllData.cusEmail.toString());
    NewVisitplanController.datafromlead.add(leadOpenAllData.state.toString());
    NewVisitplanController.datafromlead
        .add(leadOpenAllData.cusgroup.toString());

    NewVisitplanController.datafromlead.add(leadOpenAllData.area.toString());
    NewVisitplanController.datafromlead
  ...
  mapValuestoorder(GetAllLeadData leadOpenAllData, BuildContext context) async {
    ...
    await GetLeadQTHApi.getData(leadOpenAllData.LeadNum.toString())
        .then((value) async {
      if (value.stcode! >= 200 && value.stcode! <= 210) {
        for (int ik = 0;
