...
import 'package:sellerkit/DBModel/customer_model.dart';
import 'package:sellerkit/DBModel/screenshot_model.dart';
...
static Future<List<Map<String, Object?>>> getvisitftr(
      String column, Database db) async {
    final List<Map<String, Object?>> result = await db.rawQuery('''
SELECT DISTINCT $column
 FROM $tablevisitplanfilter
WHERE $column IS NOT '';
''');
...
static Future<List<Map<String, Object?>>> getsiteinftr(
      String column, Database db) async {
    final List<Map<String, Object?>> result = await db.rawQuery('''
SELECT DISTINCT $column
 FROM $tablesiteinfilter
WHERE $column IS NOT '';
''');
...
""");
 ...
 static Future<List<getvisitdetails>> getvisitplanfilter(
      Database db) async {
    final List<Map<String, Object?>> result = await db.rawQuery("""
 SELECT * From $tablevisitplanfilter 
""");
    log("result::::" + result.length.toString());
    return List.generate(result.length, (i) {
      return getvisitdetails(
        ...
        createdby:result[i]['Createdby']==null?0: int.parse (result[i]['Createdby'].toString()), 
        customercode: result[i]['Customercode'].toString(), 
        customername: result[i]['Customername'].toString(), 
        meetingtime: result[i]['Meetingtime'].toString(), 
        product: result[i]['Product'].toString(), 
        purposeofvisit: result[i]['Purposeofvisit'].toString(), 
        userid:result[i]['Userid']==null?0: int.parse(result[i]['Userid'].toString()) , 
        visitplan:result[i]['Visitplan']==null?0: int.parse(result[i]['Visitplan'].toString()) , 
        visitstatus: result[i]['VisitStatus'].toString(), 
        pincode: result[i]['Pincode'].toString(), 
        state: result[i]['State'].toString(), 
        cusmobile: result[i]['Cusmobile'].toString(), 
        cusemail: result[i]['Cusemail'].toString(), 
        contactname: result[i]['Contactname'].toString(), 
        AssignedTo: result[i]['AssignedTo'].toString(), 
        Att1: result[i]['Att1'].toString(), 
        Att2: result[i]['Att2'].toString(), 
        Att3: result[i]['Att3'].toString(), 
        Att4: result[i]['Att4'].toString(), 
        BaseId:result[i]['baseId'] ==null?0: int.parse(result[i]['baseId'].toString()), 
        BaseType:result[i]['BaseType']==null?0: int.parse(result[i]['BaseType'].toString()) , 
        CheckinDateTime: result[i]['CheckinDateTime'].toString(), 
        CheckinLat:double.parse(result[i]['CheckinLat'].toString()) , 
        CheckinLong:double.parse(result[i]['CheckinLong'].toString()) , 
        CheckoutDateTime: result[i]['CheckoutDateTime'].toString(), 
        CheckoutLat:double.parse(result[i]['CheckoutLat'].toString()) , 
        CheckoutLong:double.parse(result[i]['CheckoutLong'].toString()) , 
        ...
        PotentialBusinessValue:double.parse(result[i]['PotentialBusinessValue'].toString()) , 
        TargetId: result[i]['TargetId'].toString(), 
        TargetType: result[i]['TargetType'].toString(), 
        UpdatedBy:int.parse(result[i]['UpdatedBy'].toString()) , 
        UpdatedDateTime: result[i]['UpdatedDateTime'].toString(), 
        VisitOutcome: result[i]['VisitOutcome'].toString(), 
        VisitStatus: result[i]['VisitStatus'].toString(), 
        area: result[i]['Area'].toString(), 
        country: result[i]['Country'].toString(), 
        district: result[i]['District'].toString(), 
        plannedDate: result[i]['PlannedDate'].toString(), 
        storecode: result[i]['Storecode'].toString(), 
  ...
  static Future<List<getvisitdetails>> getsiteinfilter(
      Database db) async {
    final List<Map<String, Object?>> result = await db.rawQuery("""
 SELECT * From $tablesiteinfilter 
""");
    log("result::::" + result.length.toString());
    return List.generate(result.length, (i) {
      return getvisitdetails(
...
static Future<List<outstandingDBModel>> onsearchapply(
      Database db,
      String searchdata,
      ) async {
    final List<Map<String, Object?>> result = await db.rawQuery("""
 ...
 sum(AmountPaid) AmountPaid,sum(BalanceToPay) BalanceToPay  from Outstandingline A
      Where 
       (
        ...
        A.CustomerName LIKE '%' || '$searchdata' || '%' OR
        A.CustomerCode LIKE '%' || '$searchdata' || '%' OR
        A.TransNum LIKE '%' || '$searchdata' || '%' OR
        A.TransRef1 LIKE '%' || '$searchdata' || '%' 
...
""");
 ...
 sum(AmountPaid) AmountPaid,sum(BalanceToPay) BalanceToPay  from Outstandingline A
    ...
    Instr(case when length('$cusname') <> 0 then ', ' || '$cusname' || ',' else ', '|| A.CustomerName || ','   end,', ' || A.CustomerName || ',' ) > 0  AnD
       ...
       case when length('$tranref') <> 0 then LOWER(A.TransNum) else '${tranref.toLowerCase()}' end = '${tranref.toLowerCase()}' AnD
	  case when length('$loanref') <> 0 then LOWER(A.LoanRef) else '${loanref.toLowerCase()}' end = '${loanref.toLowerCase()}' 
...
static Future<List<getvisitdetails>> getvisitfilterapply(Database db,String assignto,String lookingfor,String enqstatus,String ordertype,double? balanceabove,String fromdate,String enddate)async{

   final List<Map<String, Object?>> result= await db.rawQuery("""
   SELECT *  from tablevisitplan A
      Where 
    Instr(case when length('$assignto') <> 0 then ', ' || '$assignto' || ',' else ', '|| A.AssignedTo || ','  end,', ' || A.AssignedTo || ',' ) > 0 AND
	  Instr(case when length('$lookingfor') <> 0 then ', ' || '$lookingfor' || ',' else ', '|| A.LookingFor || ','   end,', ' || A.LookingFor || ',' ) > 0  AnD
    
	 Instr(case when length('$enqstatus') <> 0 then ', ' || '$enqstatus' || ',' else ', '|| A.VisitStatus || ','   end,', ' || A.VisitStatus || ',' ) > 0  AnD
	
	  date(A.Meetingtime) BETWEEN '$fromdate' AND '$enddate'
...
static Future<List<getvisitdetails>> getsiteinfilterapply(Database db,String assignto,String lookingfor,String enqstatus,String ordertype,double? balanceabove,String fromdate,String enddate)async{

   final List<Map<String, Object?>> result= await db.rawQuery("""
   SELECT *  from tablesitein A
      Where 
    Instr(case when length('$assignto') <> 0 then ', ' || '$assignto' || ',' else ', '|| A.AssignedTo || ','  end,', ' || A.AssignedTo || ',' ) > 0 AND
	  Instr(case when length('$lookingfor') <> 0 then ', ' || '$lookingfor' || ',' else ', '|| A.LookingFor || ','   end,', ' || A.LookingFor || ',' ) > 0  AnD
...
""");
 ...
 static Future insertOrderdata(
  ...
  static Future insertvisitplandata(
      List<getvisitdetails> values, Database db) async {
  ...
  log("start DBBBB");
    ...
    data.forEach((es) async {
      batch.insert(tablevisitplanfilter, es);
      
    });
    
    log("tablevisitplanfilter..."+tablevisitplanfilter.length.toString());
    await batch.commit();
     stopwatch.stop();
            log('API DB ${stopwatch.elapsedMilliseconds} milliseconds');
  ...
  static Future insertsiteindata(
      List<getvisitdetails> values, Database db) async {
    
  final stopwatch = Stopwatch()..start();
  log("start DBBBB");
    var data = values.map((e) => e.toMap2()).toList();
    ...
    data.forEach((es) async {
      batch.insert(tablesiteinfilter, es);
      
    });
    
    log("tablesiteinfilter..."+tablesiteinfilter.length.toString());
    await batch.commit();
     stopwatch.stop();
            log('API DB ${stopwatch.elapsedMilliseconds} milliseconds');
...
''');
  ...
  static Future<void> truncarevisitplanfilter(Database db) async {
    await db.rawQuery('delete from $tablevisitplanfilter');
  }
  static Future<void> truncaresiteinfilter(Database db) async {
    await db.rawQuery('delete from $tablesiteinfilter');
  }
